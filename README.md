...
**v1.5.8** — 482 tests passing in CI on Linux + macOS + Windows (Git Bash), including end-to-end runtime probes of the generated shim against synthetic Claude Code stdin. 7 lifecycle hooks + 1 opt-in git pre-commit hook, 12 slash commands, 1 CLI binary, all wired through the Claude Code plugin spec.
