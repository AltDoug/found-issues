...
### Added
- `annotate-pr --pick <path:line>[,<path:line>...]` — annotate exactly the selected entries by location token, independent of file-level matching (a fix can land in a different file than the symptom). Unknown picks are reported, never silently dropped.
...
### Changed
- `annotate-pr` no longer auto-annotates ambiguous matches. File-level matching alone over-annotates: a PR touching a hot file tagged every `[open]` entry citing that file, and sync then false-flipped them all to `[fixed]` on merge (production hit 2026-07-09: `annotate-pr 102` tagged 12 entries, 3 actually fixed, 9 manually de-annotated). Now a touched file cited by exactly one entry auto-annotates as before; a file cited by several entries annotates none of them — the candidates are listed with locations + symptoms for symptom-level selection via `--pick`/`--all`. `/found-issues:annotate-pr` instructs the invoking agent to compare symptoms against the PR diff before picking.
...
- `fi_parse_entry`'s location charset now matches what `found-issues log` accepts (`[^:[:space:]]+`): paths with characters like `+` (`src/UIView+Ext.swift:42`) previously logged fine but round-tripped with an empty path — dedup silently double-logged, annotate-pr/annotate-commit never matched, tombstone sync never fired.
- `fi_repo_id` preserves dotted repo names: the `[^/.]+` capture truncated `vercel/next.js` to `vercel/next`, writing wrong slugs into `(PR: ...)` annotations so sync queried the wrong repo. `.git` is stripped as a literal suffix first, then the last two path segments are taken verbatim (same fix applied to the duplicated regex in `lib/detect-mode.sh`).
...
- `pre-branch-delete` catches the three delete forms the fixed-order patterns missed — `git branch --delete NAME`, `git push --delete REMOTE NAME`, and `git push REMOTE -d NAME` — by tokenizing the argument stream and matching the delete flag position-independently.
- `pre-commit.sh` ports format-enforcer rule 6: `[fixed]` lines must carry a verification token (`(PR: org/repo#N)`, `(commit: <sha>)`, `(verified: ai|review)`, or `(closure: tombstone)`) — manual `[open]`→`[fixed]` flips committed outside Claude Code previously bypassed exactly the workflow the enforcer blocks.
- Format-enforcer (and the pre-commit mirror) rule 1 strips canonical `(PR: org/repo#N)` annotations before the bare-`PR #N` check — a bare ref coexisting with a canonical annotation previously passed both hooks while staying invisible to sync and the statusline, a pattern `docs/format-spec.md` declares blocked.
...
### Internal
- 41 new regression tests (523 total): counter overlap/conflict/anchor cases, no-trailing-newline tombstones, plus-charset round-trip, dotted-repo annotation, spaced-path autosync dispatch (CLI + hook), `__FI_SEG`-aware splice, annotate-pr selection matrix, session-start hook-sync classifier parity + v1.5.x migrations + injection fencing, missed branch-delete forms, pre-commit rule parity.
