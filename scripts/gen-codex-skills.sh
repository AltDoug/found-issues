...
for cmd in commands/*.md; do
  ...
  # through the same Claude-only rewrites, plus two narrow fixes for text
  # that names Claude Code's own `/plugin` command family or the Claude-only
  # `/fi` alias (as of v2.5.0: commands/uninstall.md's description and
  # codex-description, and commands/setup.md's codex-description). The
  # rewrites are correct wherever those patterns appear — their whole job is
  # stripping Claude-only slash surfaces — and a no-op on every other
  # description.
