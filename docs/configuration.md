...
| `FOUND_ISSUES_DEBUG_BIN` | (unset) | Any non-empty value makes the SessionStart hook print the CLI it resolved (`found-issues: hook resolved FI_BIN=...`) to stderr. Diagnostic only — never changes which binary is chosen; `FOUND_ISSUES_BIN` is the actual override. Added in v2.2.2 because the choice is invisible otherwise and the answer surprises people: a `found-issues` on `PATH` (the **installed** plugin) outranks the co-located `FI_BIN_DIR` binary, so running the hook from a source checkout silently exercises the installed version and a local change looks like it had no effect. Claude Code injects a version-pinned plugin bin dir at session start, so a session started before a release keeps resolving the OLD version until it restarts — including that session's auto-sync. |
