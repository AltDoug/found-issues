...
**Status:** 3 of 4 groups shipped and **all merged** — v2.2.5 (#144 `c0c1baf`), v2.2.6 (#145 `9c5a9d8`), v2.2.7 (#146 `4a16a9e`). `bin/found-issues` is **5,209 → 1,466** lines. **Two PRs remain:** the operator-authorised `cmd_sync` tombstone fix, then the last extraction group — which clears the 500 signal, deletes the `loc-override` marker, and closes the ledger entry.
...
2. Merged tonight: **v2.2.5** (#144, `c0c1baf`), **v2.2.6** (#145, `9c5a9d8`), **v2.2.7** (#146, `4a16a9e`). Confirm the post-merge macOS + Windows matrices for 2.2.6 (run `31564662186`) and 2.2.7 (run `31567113734`) went green.
3. **Two PRs remain.** First fix the `cmd_sync` tombstone bug (`bin/found-issues:2215`) in its own PR — operator-authorised 2026-08-12, hazard 1 has the shape. Then the extraction: move `bin/found-issues` lines **271–1404** into four `lib/` files, delete the `loc-override` marker at `bin/found-issues:7-12`, and close the entry with `annotate-pr <N> --pick bin/found-issues:4811`.
...
1. **`cmd_sync` false-closes the tracking entry every time the CLI shrinks. This is the blocker.**
   ...
   **Operator decision, 2026-08-12: FIX IT FIRST, before the final extraction PR.** Do it narrowly — treat a present-but-shorter file as *line drift*, not closure, matching what the rename check at `:2199` and the `awk END{NR}` fix at `:2211` already do for the same false-close family. Keep tombstoning when the file is genuinely gone or renamed. Its own PR (TDD: a failing test that shrinks a file below a cited line and asserts the entry stays `[open]`), then the final extraction PR on top. Close `bin/found-issues:2215` with that PR's `annotate-pr --pick`.
...
>
> Second — the operator decided this on 2026-08-12, do not re-ask: fix the
> cmd_sync tombstone bug (bin/found-issues:2215) in its OWN PR before the
> final extraction. Narrow fix — a file that still exists but is shorter than
> the cited line is LINE DRIFT, not a closure; keep tombstoning when the file
> is genuinely gone or renamed, as the :2199 rename branch already does. TDD:
> write the failing test first (shrink a file below a cited line, assert the
> entry stays [open]). Close that entry with its own annotate-pr --pick.
>
...
>
> No decisions are outstanding — the tombstone fix is already authorised
> above. Drive both PRs to done.
