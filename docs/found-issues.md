...
- [open] 2026-05-09 .github/workflows/test.yml — no CI guard against non-ASCII test names; bats on Git Bash can't parse multi-byte UTF-8 in @test names and silently drops the test, which only manifests as a 'expected N got N-1' Windows runner failure (suggested: add a lint step using rg pattern '@test ".*[^[:ascii:]]' tests/ that fails the build with a clear error) (PR: AltDoug/found-issues#40)
